...
$App = strpos($_SERVER['HTTP_USER_AGENT'],"AFeeL") !== false;
...
if( $cert_enc_use == "Y" )
{
    ...
    if( $res_cd == "0000" )
    {
        ...
        if(!$phone_no){
			if("<?=$App?>"){
				echo "<script>alert('정상적인 인증이 아닙니다. 올바른 방법으로 이용해 주세요.');location.href='http://boosro.com/bbs/register_form.php';</script>";
			}else{
        ...
        if ($row['mb_id']) {
			if("<?=$App?>"){
				echo "<script>alert('입력하신 본인학인 정보로 가입된 내역이 존재합니다.\\n회원아이디 : ".$row['mb_id']."');location.href='http://boosro.com/bbs/register_form.php';</script>";
			}else{
    ...
    else if( $res_cd != "0000" )
    {
        // 인증실패
		if("<?=$App?>"){
			echo "<script>alert('코드 : ".$_POST['res_cd']."  ".urldecode($_POST['res_msg'])."');location.href='http://boosro.com/bbs/register_form.php';</script>";
		}else{
...
$(function() {
	...
	if("<?=$App?>"){
		location.href='http://boosro.com/bbs/register_form.php?certify_chk=true&mb_name=<?=$user_name?>&mb_hp=<?=$phone_no?>&cert_no=<?=$md5_cert_no?>';
	}else{
		window.close();
	}
});
