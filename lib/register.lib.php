...
function valid_mb_hp($reg_mb_hp)
{
    ...
    if(!$reg_mb_hp)
        ...
        if(preg_match("/^[0-9]{9,11}$/", $reg_mb_hp))
            return "";
        else
            return "请确认手机号码";
    }
...
function exist_mb_hp($reg_mb_hp, $reg_mb_id)
{
    ...
    $sql = "select count(*) as cnt from {$g5['member_table']} where (mb_hp = '$reg_mb_hp' or mb_hp = '$mb_hp') and mb_id <> '$reg_mb_id' ";
    $row = sql_fetch($sql);

    if($row['cnt'])
        return " 이미 사용 중인 휴대폰번호입니다. ";
    else
        return "";
}
...
function exist_code($code){
    global $g5;
    if (!trim($code)) return "";
	$sql = "select code from `gsw_code` where code='{$code}';";
    $row = sql_fetch($sql);
    if(!$row['code'])
        return "请确认会员代码 ";
    else
